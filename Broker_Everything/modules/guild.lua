...
local isClassicWow = select(4,GetBuildInfo())
...
local function updateGuild()
	...
	-- no stangings in Classic
	if GetGuildInfo("player") and isClassicWow then
		tmp[gName] = GetGuildInfo("player");
		tmp[gDesc], player[pStanding], player[pStandingMin], player[pStandingMax], player[pStandingValue] = "",1,1,1,1
		player[pStandingText] = "";
	else
...
local function updateApplicants()
	if isClassicWow then return end
	local temp = {};
